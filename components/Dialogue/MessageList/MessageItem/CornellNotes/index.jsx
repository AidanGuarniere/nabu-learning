...
import CopyButton from "../AssistantMessage/CopyButton";
            ...
            {noteObject.cuesAndResponses.map((item, i) => (
              <div className="w-full flex border-b border-gray-200" key={i}>
                <div className="w-1/3 border-r border-gray-200 flex justify-center md:justify-start items-center">
                  <span className="text-center md:text-left text-lg text-black">{item.cue}</span>
                </div>
                <div className="w-2/3 py-2 px-3">
                  <AssistantMessage message={item.response} isNote={true}/>
                </div>
              </div>
            ))}
          </div>
